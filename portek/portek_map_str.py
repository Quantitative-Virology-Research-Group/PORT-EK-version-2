...
class MappingPipelineString(BasePipeline):
    ...
    def _check_index(self, max_mismatches: int) -> bool:
        return pathlib.Path(
            f"{self.project_dir}/temp/ref_index/{self.ref_seq_name}_index_{self.k}_{max_mismatches}_str.pkl"
        ).exists()
    ...
    def _load_index(self, max_del_distance, verbose):
        ...
        with open(
            f"{self.project_dir}/temp/ref_index/{self.ref_seq_name}_index_{self.k}_{max_del_distance}_str.pkl",
            mode="rb",
        ) as in_file:
