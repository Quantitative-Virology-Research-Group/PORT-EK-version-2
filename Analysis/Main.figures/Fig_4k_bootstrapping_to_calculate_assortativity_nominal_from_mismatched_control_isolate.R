calculation.assortativity.nominal <- function(df) {
  df.input <- df %>% dplyr::select(isolate.x, isolate.y, distance)
  
  df.isolate.x <- df %>% dplyr::select(isolate.x, subtype) %>% dplyr::rename(isolate = isolate.x)
  df.isolate.y <- df %>% dplyr::select(isolate.y, subtype) %>% dplyr::rename(isolate = isolate.y)
...
calculation.assortativity.nominal.subset <- function(df) {
  df.subset.1 <- df %>% dplyr::filter(subset == "1")
  df.subset.1.assort.nominal <- calculation.assortativity.nominal(df.subset.1)
  
  df.subset.2 <- df %>% dplyr::filter(subset == "2")
  df.subset.2.assort.nominal <- calculation.assortativity.nominal(df.subset.2)
  
  df.subset.3 <- df %>% dplyr::filter(subset == "3")
  df.subset.3.assort.nominal <- calculation.assortativity.nominal(df.subset.3)
  
  df.subset.4 <- df %>% dplyr::filter(subset == "4")
  df.subset.4.assort.nominal <- calculation.assortativity.nominal(df.subset.4)
  
  df.subset.5 <- df %>% dplyr::filter(subset == "5")
  df.subset.5.assort.nominal <- calculation.assortativity.nominal(df.subset.5)
  
  df.subset.6 <- df %>% dplyr::filter(subset == "6")
  df.subset.6.assort.nominal <- calculation.assortativity.nominal(df.subset.6)
  
  df.subset.7 <- df %>% dplyr::filter(subset == "7")
  df.subset.7.assort.nominal <- calculation.assortativity.nominal(df.subset.7)
  
  df.subset.8 <- df %>% dplyr::filter(subset == "8")
  df.subset.8.assort.nominal <- calculation.assortativity.nominal(df.subset.8)
  
  df.subset.9 <- df %>% dplyr::filter(subset == "9")
  df.subset.9.assort.nominal <- calculation.assortativity.nominal(df.subset.9)
  
  df.subset.10 <- df %>% dplyr::filter(subset == "10")
  df.subset.10.assort.nominal <- calculation.assortativity.nominal(df.subset.10)
